...
<script>
    ...
    function handleGlobalPaste(event) {
        // Only handle if our paste area is focused or if no other input is focused
        const activeElement = document.activeElement;
        if (activeElement === pasteArea || !activeElement || activeElement === document.body) {
            handlePaste(event);
        }
    }
    
    // Mount and cleanup global listener
